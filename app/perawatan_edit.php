<?php include_once './template/header.php'; ?>
<?php include_once './template/sidebar.php'; ?>
<?php include_once './template/navbar.php'; ?>

<?php 
$perawatan = QueryOnedata("SELECT * FROM perawatan WHERE id_perawatan = ".$_GET['id_perawatan']." ")->fetch_assoc();
?>

<!-- Begin Page Content -->
<div class="container-fluid">

    <!-- Page Heading -->
    <h1 class="h3 mb-4 text-gray-800">Perawatan Page</h1>

    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">
                <a href="<?= $url ?>/app/perawatan.php" class="btn btn-info btn-sm btn-circle">
                    <i class="fas fa-arrow-left"></i>
                </a>
                Edit Data Perawatan
            </h6>
        </div>
        <div class="card-body">
            <form action="<?= $url ?>/app/aksi/perawatan.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" class="form-control" id="inputusername" name="id_perawatan" value="<?= $perawatan['id_perawatan'] ?>" required>
                <input type="hidden" class="form-control" id="id_pengguna" name="id_pengguna" value="<?= $_SESSION['id_pengguna'] ?>" required>
            
                <div class="mb-3 row">
                    <label for="inputhakses" class="col-sm-2 col-form-label">Alat Berat</label>
                    <div class="col-sm-10">
                        <select class="form-control" name="hakakses" id="inputhakses">
                            <?php
                            foreach(QueryManyData("SELECT * FROM alat_berat ") as $row) {
                                if($row["id_alat_berat"] == $perawatan['id_alat_berat']){
                                    ?>
                                    <option value="<?= $row["id_alat_berat"] ?>" selected><?= $row["nama_alat_berat"] ?></option>
                                <?php
                                }else{
                                    ?>
                                    <option value="<?= $row["id_alat_berat"] ?>"><?= $row["nama_alat_berat"] ?></option>
                                <?php
                                }                           
                            }
                            ?>
                        </select>                        
                    </div>
                </div>
                <div class="mb-3 row">
                    <label for="tanggal_perawatan" class="col-sm-2 col-form-label">Tanggal Perawatan</label>
                    <div class="col-sm-10">
                        <input type="date" class="form-control" id="tanggal_perawatan" name="tanggal_perawatan" value="<?= $perawatan['tanggal_perawatan'] ?>" required>
                    </div>
                </div>
                <div class="mb-3 row">
                    <label for="jenis_perawatan" class="col-sm-2 col-form-label">Jenis Perawatan</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="jenis_perawatan" name="jenis_perawatan" value="<?= $perawatan['jenis_perawatan'] ?>" required>
                    </div>
                </div>
                <div class="mb-3 row">
                    <label for="biaya_perawatan" class="col-sm-2 col-form-label">Biaya Perawatan</label>
                    <div class="col-sm-10">
                        <input type="number" class="form-control" id="biaya_perawatan" name="biaya_perawatan" value="<?= $perawatan['biaya_perawatan'] ?>" required>
                    </div>
                </div>
                <div class="mb-3 row">
                    <label for="keterangan" class="col-sm-2 col-form-label">Keterangan</label>
                    <div class="col-sm-10">
                        <textarea type="number" class="form-control" id="keterangan" name="keterangan" required><?= $perawatan['keterangan'] ?></textarea>
                    </div>
                </div>

                <div class="mb-3 row">
                    <button type="submit" name="updateperawatan" value="updateperawatan" class="btn btn-success btn-user btn-block">
                        <i class="fas fa-edit"></i> UPDATE
                    </button>
                </div>
            </form>
        </div>
    </div>





</div>
<!-- /.container-fluid -->

<?php include_once './template/footer.php'; ?>